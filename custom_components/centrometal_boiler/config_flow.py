...
async def try_connection(email, password):
    _LOGGER.debug(f"Trying to connect to Centrometal boiler server during setup {email}")
    ...
    if not gotConfiguration:
        raise Exception(f"Getting devices from Centrometal boiler server failed {email}")
